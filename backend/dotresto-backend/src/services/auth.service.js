...
exports.signUpDB = async (bizName, username, password) => {
    ...
    try {
        ...
        await conn.query(`INSERT INTO store_details (tenant_id, currency) VALUES (?, 'GNF')`, [result.insertId]);

        await conn.query(
            `INSERT INTO payment_types (title, is_active, tenant_id, icon) VALUES (?, 1, ?, ?), (?, 1, ?, ?), (?, 1, ?, ?)`,
            ['Cash', result.insertId, 'cash', 'Mobile Money', result.insertId, 'wallet', 'Carte bancaire', result.insertId, 'card']
        );
    } catch (error) {
        console.error(error);
        throw error;
