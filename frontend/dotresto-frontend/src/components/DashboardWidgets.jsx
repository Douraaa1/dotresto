...
export function RevenueTrendChart({ data, currencySymbol }) {
  ...
  const categories = useMemo(() => {
    return (data || []).map((d) => {
      ...
      return date.toLocaleDateString(i18n.language, { weekday: "short", month: "short", day: "numeric" });
    });
  }, [data, i18n.language]);

  const series = useMemo(() => {
    return [{ name: t("dashboard.revenue_series_name"), data: (data || []).map((d) => Number(d.revenue) || 0) }];
  }, [data, t]);
  ...
  if (!data || data.length === 0) {
    return (
      <div className="rounded-2xl border border-restro-border-green bg-background p-6">
        ...
        <p className="mt-4 text-sm text-restro-text">{t("dashboard.no_revenue_data")}</p>
      </div>
    );
  }
...
export function PeakHoursChart({ data, currencySymbol }) {
  ...
  const series = [{ name: t("dashboard.orders"), data: fullHours.map((h) => h.orders) }];
  ...
  if (!data || data.length === 0) {
    return (
      <div className="rounded-2xl border border-restro-border-green bg-background p-6 h-full">
        ...
        <p className="mt-4 text-sm text-restro-text">{t("dashboard.no_sales_data_today")}</p>
      </div>
    );
  }
  ...
  const options = {
    ...
    tooltip: {
      ...chartBaseConfig.tooltip,
      y: { formatter: (val) => `${val} ${t("dashboard.orders_tooltip_suffix")}` },
    },
    grid: {
      borderColor: "rgba(156, 163, 175, 0.15)",
      strokeDashArray: 4,
  ...
  if (!data || data.length === 0) {
    return (
      <div className="rounded-2xl border border-restro-border-green bg-background p-6 h-full">
        ...
        <p className="mt-4 text-sm text-restro-text">{t("dashboard.no_data_available")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-restro-border-green bg-background p-6 h-full">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-bold text-foreground">{t("dashboard.top_selling_items")}</h3>
        <span className="text-xs font-semibold text-restro-text">{t("dashboard.today")}</span>
      </div>
