import LogoUrl from "../assets/logo-dotresto.png";
...
export async function exportReportToExcel(report, currency) {
  ...
  const summarySheet = utils.aoa_to_sheet([...brandRows, ["Metric", "Value"], ...summaryRows.map((row) => [row.Metric, row.Value]), [], ["Powered by DotResto"]]);
  ...
  (report.tables || []).forEach((table) => {
    ...
    const sheet = utils.aoa_to_sheet([...brandRows, headers.length ? headers : ["No data"], ...body, [], ["Powered by DotResto"]]);
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <View>
            ...
            <Text style={styles.badge}>DotResto Report</Text>
          </View>
        ...
        <View style={styles.footer} fixed>
          <Text>Powered by DotResto</Text>
          <Text>Generated from DotResto SaaS</Text>
        </View>
      </Page>
    </Document>
  );
