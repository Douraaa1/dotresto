    ...
    <Page>
      <div className="px-1 pb-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2 mb-8">
          ...
          >
            {t("dashboard.view_reports")} <IconChevronRight size={16} stroke={iconStroke} />
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
          <KPICard
            label={t("dashboard.kpi_todays_revenue")}
          ...
          />
          <KPICard
            label={t("dashboard.kpi_avg_order_value")}
          ...
          />
          <KPICard
            label={t("dashboard.kpi_new_customers")}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="rounded-xl border border-restro-border-green bg-background p-4 text-center">
            ...
            <p className="text-xs font-semibold text-restro-text mt-1">{t("dashboard.repeat_customers")}</p>
          </div>
          <div className="rounded-xl border border-restro-border-green bg-background p-4 text-center">
            <p className="text-2xl font-extrabold text-foreground">{cancelledOrders || 0}</p>
            <p className="text-xs font-semibold text-restro-text mt-1">{t("dashboard.cancelled_orders")}</p>
          </div>
          <div className="rounded-xl border border-restro-border-green bg-background p-4 text-center">
            <p className="text-2xl font-extrabold text-foreground">
              {sym}{Number(todayRevenue?.tax_total || 0).toFixed(0)}
            </p>
            <p className="text-xs font-semibold text-restro-text mt-1">{t("dashboard.tax_collected")}</p>
          </div>
          <div className="rounded-xl border border-restro-border-green bg-background p-4 text-center">
            <p className="text-2xl font-extrabold text-foreground">
              {sym}{Number(todayRevenue?.service_charge_total || 0).toFixed(0)}
            </p>
            <p className="text-xs font-semibold text-restro-text mt-1">{t("dashboard.service_charge")}</p>
          </div>
        </div>
      </div>
    </Page>
